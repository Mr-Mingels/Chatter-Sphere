...
router.put("/send-friend-request", async (req, res) => {
  ...
  try {
    ...
    if (
      !mongoose.Types.ObjectId.isValid(requestedFriendUserId) ||
      requestedFriendUserId === userId
    ) {
    ...
    const checkSentRequests = userByFriendId.sentFriendRequests.find(
      (id) => id.toString() === userId
    );
    const checkRecievedRequests = userByFriendId.receivedFriendRequests.find(
      (id) => id.toString() === userId
    );
    const checkFriendsList = userByFriendId.friends.find(
      (id) => id.toString() === userId
    );
    if (checkSentRequests) {
      await User.updateOne(
        { _id: requestedFriendUserId },
        { $pull: { sentFriendRequests: userId } }
      );
      await User.updateOne(
        { _id: userId },
        { $pull: { receivedFriendRequests: requestedFriendUserId } }
      );
      await User.updateOne(
        { _id: userId },
        { $push: { friends: requestedFriendUserId } }
      );
      await User.updateOne(
        { _id: requestedFriendUserId },
        { $push: { friends: userId } }
      ...
      io.to(userId).emit(
        ...
        userByFriendId.username
      );
      io.to(requestedFriendUserId).emit(
        "friendRequestAccepted",
        requestedFriendUserId,
        userByFriendId.username
      );
      return res
        .status(200)
        .send({ message: `They've already sent a request. Added!` });
    } else if (checkRecievedRequests) {
      return res
        .status(400)
        .send({ message: `You've already sent them a friend request` });
    ...
    await User.updateOne(
      { _id: userId },
      { $push: { sentFriendRequests: requestedFriendUserId } }
    );
    io.to(userId).emit(
  ...
  try {
    ...
    if (userById) {
      ...
      for (let i = 0; i < userById.sentFriendRequests.length; i++) {
        const userBySentId = await User.findOne({
          _id: userById.sentFriendRequests[i],
        });
...
router.get("/received-friend-requests", async (req, res) => {
  try {
    ...
    const userById = await User.findOne({ _id: userId });
    if (userById) {
      const usersReceivedRequests = await User.find({
        _id: { $in: userById.receivedFriendRequests },
      });
...
router.put("/accept-friend-request", async (req, res) => {
  ...
  try {
    ...
    const userId = req.user._id.toString();
    const userById = await User.findOne({ _id: userId });
    const recievedFriendUserById = await User.findOne({
      _id: receivedFriendUserId,
    });
    const checkSentRequests = recievedFriendUserById.sentFriendRequests.find(
      (id) => id.toString() === userId
    );
    const checkRecievedRequests = userById.receivedFriendRequests.find(
      (id) => id.toString() === receivedFriendUserId
    );
    if (checkSentRequests || checkRecievedRequests) {
      ...
      await User.updateOne(
        { _id: userId },
        { $pull: { receivedFriendRequests: receivedFriendUserId } }
      );
    }

    await User.updateOne(
      { _id: userId },
      { $push: { friends: receivedFriendUserId } }
    );
    await User.updateOne(
      { _id: receivedFriendUserId },
      { $push: { friends: userId } }
    ...
    const usersAlreadyHaveAChat = await Chat.findOne({
    ...
    if (usersAlreadyHaveAChat && !usersAlreadyHaveAChat.isGroupChat) {
      io.to(userId).emit(
        "friendRequestAccepted",
        receivedFriendUserId,
        recievedFriendUserById.username
      );
      io.to(receivedFriendUserId).emit(
        "friendRequestAccepted",
        receivedFriendUserId,
        recievedFriendUserById.username
      );
...
router.put("/unsend-friend-request", async (req, res) => {
  ...
  try {
    ...
    const checkSentRequests = userById.sentFriendRequests.find(
      (id) => id.toString() === receivedFriendUserId
    );
    const checkRecievedRequests =
      recievedFriendUserById.receivedFriendRequests.find(
        (id) => id.toString() === userId
      );
    if (checkSentRequests || checkRecievedRequests) {
      await User.updateOne(
        { _id: userId },
        { $pull: { sentFriendRequests: receivedFriendUserId } }
      );
      await User.updateOne(
        { _id: receivedFriendUserId },
...
router.get("/friends-list", async (req, res) => {
  try {
    ...
    const userById = await User.findOne({ _id: userId });
    if (userById) {
      const usersFriendsList = await User.find({
        _id: { $in: userById.friends },
      });
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(200)
        .send({ message: "Profile Picture updated", url: profilePictureUrl });
    } catch (err) {
      console.log(err);
      res.status(500).send({ message: "Server error" });
    }
