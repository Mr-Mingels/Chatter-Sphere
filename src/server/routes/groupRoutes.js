...
router.put("/leave-group", async (req, res) => {
  ...
  try {
    ...
    const message = `${
      userById.username.charAt(0) + userById.username.slice(1).toLowerCase()
    } has left the group.`;
  ...
  try {
    ...
    if (memberIds.length !== 0 && chatId) {
      const userPromises = memberIds.map((memberId) =>
        User.findOne({ _id: memberId.toString() })
      );
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(200)
        .send({ message: "Group Picture updated", url: groupPictureUrl });
    } catch (err) {
      console.log(err);
