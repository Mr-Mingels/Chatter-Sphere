...
const Messages = ({ extractedUserInfo, extractedChatsListInfo, chatListInfoFunction, setExtractedRenderedChatMsgs, windowWidth }) => {
    ...
    const getChatMessages =  async () => {
        try {
            const response = await axios.get(`/chats/${chatId}/messages`, { withCredentials: true })
    ...
    const createMessage = async () => {
        try {
          ...
          const response = await axios.post(`/messages`, { idInfo }, { withCredentials: true });
          setMessageTxt('');
        } catch (err) {
          console.log(err);
          setMessageTxt('');
    ...
    const deleteMsg = async () => {
        try {
            const response = await axios.delete('/delete-message', { data: { selectedMsg } }, { withCredentials: true })
            closeSeveringModal()
        } catch (err) {
            console.log(err)
        }
