    ...
    <div className="removeFriendModalContent">
      <div className="removeFriendModalMainContentWrapper">
        <div className="removeFriendModalMainContent">
          <p className="removeFriendModalTxt">
            Are you sure you want to {modalText}
          </p>
        </div>
        <div className="removeFriendModalFooterWrapper">
          <button
            className="removeFriendModalBtn closeBtn"
            onClick={
              severingModalOption || selectedMsg
                ? () => closeSeveringModal()
                : () => closeModal()
            }
          >
          ...
          {modalLoader || loader ? (
            <button className="removeFriendModalBtn removeFriendLoader">
              <span className="modalLoader"></span>
            </button>
          ) : (
            <button
              className="removeFriendModalBtn red"
              onClick={
                severingModalOption === "deleteGroup"
                  ? () =>
                      deleteGroup(
                        currentChatInfo._id.toString(),
                        currentChatInfo.members
                      )
                  : severingModalOption === "leaveGroup"
                  ? () => leaveGroup(currentChatInfo._id.toString())
                  : selectedMsg
                  ? () => deleteMsg()
                  : () => removeFriend()
              }
            >
              {severingModalOption === "deleteGroup" || selectedMsg
                ? "Delete"
                : severingModalOption === "leaveGroup"
                ? "Leave"
                : "Remove"}
