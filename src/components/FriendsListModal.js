...
const FriendsListModal = ({
  ...
  const addMember = (friendId) => {
    const checkIfMember = currentChatInfo.members.find(
      (member) => member._id === friendId
    );
    ...
    <>
      <div className="friendModalHeaderWrapper">
        ...
        <div className="friendsModalSearchInputWrapper">
          <svg
            ...
            className="friendsModalSearchIcon"
            viewBox="0 0 512 512"
          >
            <path
          ...
          <input
            className="friendsModalSearchInput"
            placeholder="Search"
      ...
      <div className="friendModalMainContentWrapper">
        <div className="friendModalMainContent">
          {!friendsListModalLoader && searchedFriends ? (
            <>
              {searchedFriends.map((user, index) => (
                <div
                  className={`friendModalSentUserContentWrapper ${
                    addedMember.find((id) => id === user._id.toString())
                      ? "addedMember"
                      : ""
                  }`}
                  key={index}
                ...
                >
                  {user.profilePicture ? (
                    <img
                      ...
                      className="friendModalProfileImg"
                      alt="your friends profile pic"
                    />
                  ) : (
                    <div className="friendModalDefaultProfileImgWrapper">
                      <h3 className="friendModalDefaultProfileImg">
                  ...
                  {!addMemberOption && (
                    <button
                      className="friendModalRemoveBtn"
                      onClick={() =>
                        openRemoveFriendModal(
                          user._id.toString(),
                          user.username
            ...
            <div className="friendsListModalLoaderWrapper">
              <span className="friendsListModalLoader"></span>
            </div>
          )}
        </div>
        <div className="friendModalFooterWrapper">
          <button
            className="friendModalBtn closeBtn"
            onClick={
              addMemberOption
                ? () => closeFriendListModal()
                : () => closeModal()
          ...
          {modalLoader ? (
            <button className="friendModalBtn addMemberLoader">
              <span className="modalLoader"></span>
            </button>
          ) : (
            <button
              className="friendModalBtn"
              onClick={
                addMemberOption
                  ? () => addMemberToGroup(addedMember, currentChatInfo._id)
                  : () => openAddFriendModal()
              }
            >
              {" "}
              {addMemberOption ? "Add to Group" : "Add a Friend"}
            </button>
          )}
        </div>
      </div>
